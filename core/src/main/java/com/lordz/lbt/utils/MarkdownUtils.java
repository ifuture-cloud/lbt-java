...
public class MarkdownUtils {
    ...
    public static String renderMarkdown(String content) {
        ...
        if (content.contains(LBTConst.NETEASE_MUSIC_PREFIX)) {
            renderContent = content.replaceAll(LBTConst.NETEASE_MUSIC_REG_PATTERN, LBTConst.NETEASE_MUSIC_IFRAME);
        }

        // render bilibili video short url
        if (content.contains(LBTConst.BILIBILI_VIDEO_PREFIX)) {
            renderContent = content.replaceAll(LBTConst.BILIBILI_VIDEO_REG_PATTERN, LBTConst.BILIBILI_VIDEO_IFRAME);
        }

        // render youtube video short url
        if (content.contains(LBTConst.YOUTUBE_VIDEO_PREFIX)) {
            renderContent = content.replaceAll(LBTConst.YOUTUBE_VIDEO_REG_PATTERN, LBTConst.YOUTUBE_VIDEO_IFRAME);
        }
